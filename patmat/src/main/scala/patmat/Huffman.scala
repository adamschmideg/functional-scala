...
object Huffman {
   ...
   */
    def until[T](shouldStop: (List[T] => Boolean), shrink: (List[T] => List[T]))(coll: List[T]): List[T] =
      if (shouldStop(coll)) coll else until(shouldStop, shrink)(shrink(coll))
   ...
   */
    def createCodeTree(chars: List[Char]): CodeTree = {
      val frequencies = makeOrderedLeafList(times(chars))
      until(singleton, combine)(frequencies).head
    }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodeLetter(subtree: CodeTree, bits: List[Bit]): (List[Bit], Char) = subtree match {
      case leaf: Leaf => (bits, leaf.char)
      case fork: Fork => {
        val sub = if (bits.head == 0) fork.left else fork.right
        decodeLetter(sub, bits.tail)
      }
    }
    if (bits.isEmpty) List()
    else {
      val (restBits, char) = decodeLetter(tree, bits)
      char :: decode(tree, restBits)
    }
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def encodeChar(subtree: CodeTree, char: Char, bits: List[Bit] = List()): List[Bit] = subtree match {
      case leaf: Leaf => bits
      case fork: Fork =>  {
        val (newTree, newBits) = if (chars(fork.left).contains(char)) (fork.left, List(0)) else (fork.right, List(1))
        bits ::: encodeChar(newTree, char, newBits)
      }
    }
    text.map(encodeChar(tree, _)).reduce(_ ::: _)
  }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    text.map(c => codeBits(table)(c)).reduce(_ ::: _)
  }
}
